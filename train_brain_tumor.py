"""
End-to-end PyTorch training script for brain tumor MRI classification.

Dataset: Kaggle "Brain Tumor MRI Dataset" (Masoud Nickparvar), 4 classes:
    glioma, meningioma, notumor, pituitary.

The script auto-downloads the dataset from Kaggle if it is not already present.
...
Usage:
    ...
    python train_brain_tumor.py --kaggle_user YOUR_NAME --kaggle_key YOUR_API_KEY

    # skip download if data already exists
    python train_brain_tumor.py --data data --epochs 10 --batch 32
"""
...
def download_dataset(data_dir: Path, args):
    """
    Download the Brain Tumor MRI dataset from Kaggle into data_dir.
    Skips the download entirely if Training/ images are already on disk.
    ...
    # The archive may extract into a sub-folder — move its contents up one level
    nested = data_dir / "brain-tumor-mri-dataset"
    if nested.exists() and not training_dir.exists():
...
def index_folder(root: Path):
    """Walk Training/ and Testing/ subdirs and record every image file."""
    rows = []
        ...
        for cls_dir in sdir.iterdir():
            ...
            for p in cls_dir.glob("*"):
                try:
                    ...
                    rows.append({"path": str(p), "split": split,
                                 "label": cls_dir.name, "ok": False, "err": str(e)})
...
def main():
    ap = argparse.ArgumentParser(
        description="Brain Tumor MRI classifier — auto-downloads data from Kaggle."
    )
    # data / output
    ...
    # kaggle credentials (optional — falls back to env vars / ~/.kaggle/kaggle.json)
    ...
    if df.empty or not df["ok"].any():
        raise FileNotFoundError(
            ...
            "Check that the Kaggle dataset extracted to:\n"
            "  data/Training/<class>/*.jpg\n"
            "  data/Testing/<class>/*.jpg"
        )
    corrupt = (~df["ok"]).sum()
