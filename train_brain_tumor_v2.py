...
Usage:
    ...
    python train_brain_tumor_v2.py --kaggle_user YOUR_NAME --kaggle_key YOUR_KEY
    python train_brain_tumor_v2.py --data data --epochs 10 --batch 32
"""
...
def download_dataset(data_dir: Path, args):
    """
    Download the Brain Tumor MRI dataset via kagglehub.
    ...
    # Flatten nested subfolder if the archive extracted into one
...
def main():
    ap = argparse.ArgumentParser(
        description="Brain Tumor MRI classifier — simplified Kaggle setup via kagglehub."
    )
    ...
    if df.empty or not df["ok"].any():
        raise FileNotFoundError(
            ...
            "Check that the dataset extracted to:\n"
