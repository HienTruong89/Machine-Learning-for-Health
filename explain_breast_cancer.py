"""
Explainable AI for breast cancer ultrasound classification.

Loads the best checkpoint produced by train_breast_cancer.py and generates
...
Outputs are saved to  artifacts_breast/explanations/  as individual PNG files
and a summary HTML report that can be opened in any browser.

Test images are sourced from  artifacts_breast/test_images.csv  (written by
train_breast_cancer.py) so that explanations are always drawn from the
held-out set.  If that file is absent, all images under --data are used.
...
Usage:
    ...
    python explain_breast_cancer.py --checkpoint artifacts_breast/best_model.pt --n 16
"""
...
from train_breast_cancer import build_model, index_folder
...
def overlay_heatmap(raw_img, heatmap, alpha=0.5, cmap="jet"):
    ...
    hm_color = colormap(hm_resized)[..., :3]
...
<title>Breast Cancer XAI Report</title>
<style>
...
<h1>Explainable AI &mdash; Breast Cancer Ultrasound</h1>
...
def main():
    ap = argparse.ArgumentParser(
        description="Explainable AI for breast cancer ultrasound classification."
    )
    ap.add_argument("--checkpoint", type=str,
                    default="artifacts_breast/best_model.pt",
                    help="Path to the best model checkpoint.")
    ap.add_argument("--data", type=str, default="data_breast",
                    help="Root data directory (same as train_breast_cancer.py).")
    ...
    ap.add_argument("--out", type=str,
                    default="artifacts_breast/explanations",
    ...
    if args.image:
        ...
        # ---- Batch mode: prefer the saved test-split CSV -------------------- #
        artifacts_dir = Path(args.checkpoint).parent
        test_csv      = artifacts_dir / "test_images.csv"

        if test_csv.exists():
            print(f"Loading test images from {test_csv}")
            test_df = pd.read_csv(test_csv)
        else:
            print(f"test_images.csv not found — sampling from all images in '{args.data}'")
            full_df = index_folder(Path(args.data))
            if full_df.empty:
                print("No images found. Provide --image or --data.")
                sys.exit(1)
            test_df = full_df[full_df["ok"]].copy()

        if test_df.empty:
            print("No test images available. Run train_breast_cancer.py first.")
            sys.exit(1)
